...
Use this lens inside `feasibility` and `adversarial` when the task involves a
proposal, design, RFC, feature request, or solution that may be solving the
wrong problem. It is a maintainer deletion frame, not a separate gate and not a
general code review style pass.

- Treat every stated requirement as a hypothesis until it is tied to a concrete
  user, failure mode, maintainer cost, compatibility need, or security/data
  risk.
- Recover the real requirement before judging the proposed solution. If the
  real requirement is missing, mark that as missing evidence instead of
  ratifying the proposal.
- Try deletion before simplification. Prefer `do nothing`, close as duplicate,
  narrow scope, remove a process, or use an existing mechanism when that fully
  satisfies the real requirement.
- Simplify only what survives deletion. Do not optimize, automate, configure,
  or generalize a component that has not earned its place.
- Compare proposed surface area against the smallest acceptable path: files,
  contracts, tests, docs, review rounds, compatibility burden, and future owner
  cost.
- End with a decisive minimum sufficient solution: land, delete, narrow, defer
  pending evidence, or close. Avoid options menus unless the remaining decision
  is genuinely product policy.
...
- Apply the Requirement Deletion Lens before accepting a proposed solution.
...
- Apply the Requirement Deletion Lens when the current story depends on a
  debatable requirement, new abstraction, workflow, or automation.
